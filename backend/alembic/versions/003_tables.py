...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column("primary_guest_id", postgresql.UUID(as_uuid=True), nullable=False),
        ...
        sa.Column("guest_count", sa.SmallInteger(), nullable=False, server_default=sa.text("1")),
        sa.Column(
            "nightly_rate",
            sa.Numeric(12, 2),
            nullable=False,
        ...
        sa.ForeignKeyConstraint(
            ...
            ["guests.id"],
            ondelete="RESTRICT",
            name="fk_reservations_primary_guest_id_guests",
        ),
        sa.ForeignKeyConstraint(
            ["room_type_id"],
            ["room_types.id"],
            ondelete="RESTRICT",
            name="fk_reservations_room_type_id_room_types",
        ),
        sa.ForeignKeyConstraint(
            ["room_id"],
            ["rooms.id"],
            ondelete="SET NULL",
            name="fk_reservations_room_id_rooms",
        ),
    )
    ...
    op.create_table(
        ...
        sa.PrimaryKeyConstraint(
            "reservation_id",
            "guest_id",
            name=op.f("pk_reservation_guests"),
        ),
        sa.ForeignKeyConstraint(
            ["reservation_id"],
            ["reservations.id"],
            ondelete="CASCADE",
            name=op.f("fk_reservation_guests_reservation_id_reservations"),
        ),
        sa.ForeignKeyConstraint(
            ["guest_id"],
            ["guests.id"],
            ondelete="RESTRICT",
            name=op.f("fk_reservation_guests_guest_id_guests"),
        ),
    )
    op.create_index(
        "idx_res_guest_guest", "reservation_guests", ["guest_id"], unique=False
    )
    ...
    op.create_table(
        ...
        sa.ForeignKeyConstraint(
            ...
            ["reservations.id"],
            ondelete="CASCADE",
            name=op.f("fk_payments_reservation_id_reservations"),
        ),
        sa.UniqueConstraint("reservation_id", name="uq_payments_reservation"),
    )
    op.create_index("idx_payments_res", "payments", ["reservation_id"], unique=False)
    op.create_index(
        "idx_payments_deleted_at", "payments", ["deleted_at"], unique=False
    )
...
def downgrade() -> None:
    ...
    op.drop_index("idx_reservations_primary_guest", table_name="reservations")
